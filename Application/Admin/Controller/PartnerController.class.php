...
Class PartnerController extends CommonController{
	...
	Public function deluserform(){
		...
		if(null != I('del')){
			...
			if ($result){
				$this->success('删除成功', U('/Admin/Partner/deluser'));
			}else{
				$this->error('删除失败');
			}
		}
			...
			if ($result){
				$this->success('管理权限增加成功', U('/Admin/Partner/deluser'));
			}else{
				$this->error('管理权限增加失败');
			}
		}
			...
			if ($result){
				$this->success('管理权限删除成功', U('/Admin/Partner/deluser'));
			}else{
				$this->error('管理权限删除失败');
			}
		}
	...
	Public function addusertab(){
		...
		if($result){
			$this->success('新增成功', U('/Admin/Partner/adduser'));
		}else{
			$this->error('添加失败');
		}
	}
